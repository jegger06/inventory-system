$(document).ready(function() {
	...
	$('#per_page').on('change', function() {
		page_num = 1;
		loadWaitingUser(total_data, page_num);
		setTimeout(function(){
			loadPagination(page = 1, total_data);
		}, 300);
	});
	...
	$('body').delegate('.status', 'click', function(e) {
		...
		swal({
			title: 'Activate?',
			text: 'This will give ' + name + ' access to the system.',
			type: 'info',
				...
				$.ajax({
					...
					success: function(data) {
						if (data.success == 1) {
							...
							loadWaitingUser(total_data, page_num = 1);
							loadPagination(page = 1, total_data);
						}
					}
				});
	...
	function loadWaitingUser(total_data) {
		...
		$.ajax({
			type: 'POST',
			url: '/app/model/user/php/load_waiting_user.php',
			...
			success: function(data) {
				if (data.success == 1) {
					...
					$('.list_waiting_user tbody').html(tbl_row);
